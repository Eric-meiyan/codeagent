    ...
    await Promise.all([
      ...
      db()
        .select({
          ...
          cacheCreationInputTokens: codeBillingEvent.cacheCreationInputTokens,
...
async function sessionBillingSummary(sessionId: string) {
  const [aggregate] = await db()
    .select({
      ...
      cacheCreationInputTokens: sql<number>`coalesce(sum(${codeBillingEvent.cacheCreationInputTokens}), 0)`,
